...
def train():
    with open(LOG_FILE, 'w') as f:
        f.write("--- V8 Training (cached data + attention-cls) ---\n")
    log_print("--- Starting V8 Training ---")
    ...
    model = HintGeneratorGNN(MAX_VOCAB_SIZE, EMBED_DIM, 128, len(bug_types), num_patch_classes=NUM_PATCH_CLASSES)
    model = model.to(device)

    # Warm-start from best checkpoint if available
    ckpt_path = 'gnn_model_best.pth'
    ...
    for epoch in range(EPOCHS):
        ...
        for i, sample in enumerate(train_samples):
            try:
                loss, cls_ok, loc_ok, patch_ok, is_buggy, had_target = \
                    process_cached_sample(sample, model, device,
                                          criterion_cls, criterion_loc, criterion_patch, vocab)
        ...
        with torch.no_grad():
            for sample in val_samples:
                try:
                    _, cls_ok, loc_ok, patch_ok, is_buggy, had_target = \
                        process_cached_sample(sample, model, device,
                                              criterion_cls, criterion_loc, criterion_patch, vocab)
                    v_processed += 1
                    if cls_ok: v_cls += 1
        ...
        if v_processed > 0:
            ...
            if v_e2e_a > best_val_e2e:
                ...
                torch.save(model.state_dict(), "gnn_model_best.pth")
                with open(VOCAB_FILE, 'w') as f:
                    json.dump(vocab, f)
                log_print(f"         ** Best E2E={v_e2e_a:.1f}% — saved **")

        if (epoch + 1) % 20 == 0:
            torch.save(model.state_dict(), f"gnn_model_v8_ep{epoch+1}.pth")
