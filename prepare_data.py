"""
Pre-generate training data: multiple mutations per problem, cached CPG tensors.
Run once, then train.py loads from cache for fast, consistent training.
"""
import torch
from torch_geometric.data import Data
import json, random, pickle, os, sys
...
def _seed_mutation_tokens():
    ...
    Without this, tokens like 'True', 'False', integers are MISSING from
    the vocabulary and the patch head can never learn them (maps to UNK=0)."""
    mutation_tokens = [
        "+", "-", "*", "/", "%", "**",
        "==", "!=", "<", ">", "<=", ">=",
        "and", "or",
    ...
    # Also register small integers (off-by-one mutations)
    for i in range(-2, 20):
...
def main():
    ...
    for pi, prob in enumerate(problems):
        ...
        try:
            ...
            print(f"  Skip problem {pi}: can't build optimal graph: {e}")
            skipped += 1
            continue
        ...
        try:
            ...
            samples.append({
                't_user': t_user, 't_opt': t_opt, 'u_nodes': u_nodes,
                'target_cls': 0, 'is_buggy': False, 'metadata': None,
                'problem_idx': pi,
            })
        except Exception:
            pass
        ...
        while mutations_done < MUTATIONS_PER_PROBLEM and attempts < MUTATIONS_PER_PROBLEM * 5:
            ...
            try:
                ...
                samples.append({
                    't_user': t_user, 't_opt': t_opt, 'u_nodes': u_nodes,
                    'target_cls': target_cls, 'is_buggy': True,
                    'metadata': {
    ...
    print(f"Buggy: {buggy_count}, Clean: {len(samples) - buggy_count}")

    # Split by problem_idx to avoid data leakage
